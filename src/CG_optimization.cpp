...
public:
    ...
    inline void conjugated_gradient_minimization(const double BOX, MatrixXd& position);

    inline void trend_output(double potential_value, double gradient_squaredNorm, ofstream& fout);

    inline void coordinates_output(const double BOX, MatrixXd& position, ofstream& fout);
...
private:
    ...
    ofstream coordinates_metastate_out;             // output file stream of metastate coordinates
...
void CG::conjugated_gradient_minimization(const double BOX, MatrixXd& position) {
    ...
    // int count(0);   // count number for coordinate output
    ...
    while (gradient_squaredNorm > precision_2) {
        ...
        trend_output(potential_value, gradient_squaredNorm, trend_out);
        // coordinates_metastate_out.open(outputPath + "/coordinates_metastates_" + to_string(count) +".cif");
        // coordinates_output(BOX, position_ii, coordinates_metastate_out);
        // coordinates_metastate_out.close();
        // count++;
    }

    //record final position
    coordinates_output(BOX, position_ii, coordinates_out_CG);
...
void CG::coordinates_output(const double BOX, MatrixXd& position, ofstream& fout) {
...
int main(int argc, char* argv[]) {
    ...
    CG cg_optimization(output_path);
    cg_optimization.conjugated_gradient_minimization(BOX, ensemble);
}
